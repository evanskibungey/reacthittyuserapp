...
const Products = ({ setIsLoggedIn }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        // For now, we'll handle the scenario where the API isn't fully implemented
        try {
          ...
          setProducts([
            ...
            {
              id: 9,
              name: 'Professional Gas Torch',
              description: 'Ideal for culinary applications and DIY projects',
              selling_price: 2499,
              category_id: 5,
              category: 'Tools',
              current_stock: 18,
            ...
            {
              id: 10,
              name: 'Gas Detector with Alarm',
              description: 'Advanced safety device with audible alarm system',
              selling_price: 2299,
              category_id: 2,
              category: 'Safety Devices',
              current_stock: 12,
  ...
  const filteredProducts = products
    ...
    .filter(product => product.selling_price >= priceRange[0] && product.selling_price <= priceRange[1])
    .sort((a, b) => {
      switch (sortBy) {
        case 'price-low':
                  ...
                  .map(product => (
                    <div key={`top-${product.id}`} className="flex">
                      <div className="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                        <img 
                          src={`/api/placeholder/100/100?text=${encodeURIComponent(product.name.substring(0, 10))}`}
                          alt={product.name}
